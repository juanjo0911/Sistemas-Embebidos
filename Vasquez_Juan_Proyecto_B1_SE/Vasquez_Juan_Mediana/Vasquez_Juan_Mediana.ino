...
void loop() {
  median((double *)&ecg_100Hz[0], (double *)&output_signal2[0], (int) tam_sig,5);
  sruido2();
  plot_signal();
  delay(100);
...
void sruido2(){
  ...
  for(int i=0;i<tam_sig;i++){
    ...
    ru1[i]=pow(ecg_100Hz[i],2);
    ru2[i]=pow((ecg_100Hz[i]-output_signal2[i]),2);
    signalNR2[i]=10*log(ru1[i]/ru2[i]); 
    }
  }
